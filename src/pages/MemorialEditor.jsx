...
export default function MemorialEditor({ memorialId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: photoRows }, { data: eventRows }, { data: memberRows }] = await Promise.all([
        supabase.from("memorial_photos").select("*").eq("memorial_id", memorialId).order("sort_order"),
        supabase.from("life_events").select("*").eq("memorial_id", memorialId).order("sort_order"),
        supabase.from("family_members").select("*").eq("memorial_id", memorialId).order("sort_order"),
      ]);
  ...
  if (loading) {
    return <div className="route-loading">Cargando memorial…</div>;
  }

  if (!memorial) {
    return <p className="auth-error">No se encontró este memorial.</p>;
  }
